        ...
        <div className={`absolute top-0 flex w-full xl:!w-full items-center justify-between max-h-screen lg:!h-[10vh] ${colorChange ? 'bg-gradient-to-b from-orange-100 to-yellow-200 !shadow-lg' : ''}`}>
            <div className='h-full flex justify-start items-center'>
                <div className='flex items-center justify-center mix-blend-multiply xl:!hidden py-2 mx-2 sm:!mx-4 z-50' onClick={() => setIsOpen(true)}>
                    <img src={hamburger} alt="" className='h-12 w-12 sm:!h-16 sm:!w-16' />
                </div>
                <motion.div
            ...
            <motion.div
                ...
                <div className='flex w-full nav-list'>
                    <ul className='flex w-full'>
                        ...
                        <li className={`p-3 font-mova text-shadow text-[1.2rem] text-[--black] tracking-[.1rem] mx-3 ${colorChange ? 'hover:text-white' : 'hover:text-[--orange]'} ${activeSection === 'contact' ? 'text-white' : ''}`}><Link onClick={() => scrollToSection('contact')} to={`#${activeSection}`}>Contact</Link></li>
                    </ul>
                </div>
            </motion.div>
            {isOpen &&
